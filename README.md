...
File: train.csv (obtained from a public repository, e.g., Kaggle or data science dojo)
Methodology
...
Column Renaming: Standardized column names for clarity and consistency (e.g., Pclass to Passenger_Class, SibSp to Siblings_Spouses_Count).
Data Type Optimization: Optimized memory usage by converting columns to appropriate data types (e.g., int64 to int32/int8, object to category, float64 to float32 or Int64 for Age).
Impact: Achieved significant memory reduction (up to 87.5% for some columns).
Missing Value Handling:
Cabin_Info: Dropped due to extremely high missing rate (~77%).
Embarkation_Port: Imputed the 2 missing values with the mode ('S').
Age: Imputed 177 missing values using a group-based median imputation strategy (grouping by Passenger_Class and Gender) to preserve demographic patterns.
3. Feature Engineering
Several new features were engineered to provide more meaningful insights and improve model performance:

Title: Extracted social titles (e.g., Mr, Miss, Mrs, Master, Rare) from the Passenger_Name column.
Family_Size: Calculated as Siblings_Spouses_Count + Parents_Children_Count + 1.
Is_Alone: A binary feature indicating whether a passenger was traveling alone (Family_Size == 1).
Age_Category: Categorized Age into bins (Child, Teen, Adult, Senior).
Family_Size_Category: Categorized Family_Size into bins (Alone, Small, Medium, Large).
Log_Fare: Applied a logarithmic transformation to Ticket_Fare to reduce skewness and normalize its distribution.
Fare_Category: Categorized Log_Fare into quantiles (Low, Mid_Low, Mid_High, High).
4. Visualizations
